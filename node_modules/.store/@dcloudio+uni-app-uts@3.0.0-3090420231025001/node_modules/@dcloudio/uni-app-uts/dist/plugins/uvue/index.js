    ...
    return {
        ...
        async transform(code, id) {
            ...
            if (!query.vue) {
                ...
                if (errors.length) {
                    errors.forEach((error) => this.error((0, error_1.createRollupError)(filename, error, code)));
                    return null;
                }
...
async function transformVue(code, filename, options, pluginContext, isAppVue = () => false, normalizeEasyComSource) {
    ...
    const fileName = (0, uni_cli_shared_1.normalizePath)(path_1.default.relative(options.root, filename));
    ...
    if (!isApp) {
            ...
            onWarn(warning) {
                ...
                if (warning.loc) {
                    ...
                    console.log('at ' +
                        ...
                        ':' +
                        (start.column - 1));
                    console.log((0, uni_cli_shared_2.generateCodeFrame)(code, {
        ...
        if (process.env.NODE_ENV === 'production') {
            (0, utils_1.addExtApiComponents)(templateResult.elements);
        }
    }
    // 生成 script 文件
    ...
    const utsCode = (0, script_1.genScript)(descriptor, { filename: className }) +
        ...
        '\n';
    let jsCode = templateImportEasyComponentsCode + templateImportUTSComponentsCode;
...
export const ${(0, utils_2.genComponentPublicInstanceImported)(options.root, filename)} = {}`;
    return {
        errors: [],
        uts: utsCode,
        js: jsCode,
        descriptor,
        sourceMap: needSourceMap
            ? createSourceMap(descriptor.script?.loc.end.line ?? 0, templateStartLine, new compiler_sfc_1.MagicString(code).generateMap({
                hires: true,
                source: fileName,
                includeContent: true,
            }), templateSourceMap)
...
function createSourceMap(scriptCodeOffset, templateCodeOffset, scriptMap, templateMap) {
    if (!templateMap) {
        return scriptMap;
    }
